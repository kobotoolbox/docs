...
### الاستثناءات
هناك استثناءان للقاعدة أعلاه.

#### استثناء لنقطة نهاية exports
في `v1`، كانت نقطة نهاية `/exports/` تُرجع **جميع الصادرات للمستخدم المصادق عليه** عبر جميع المشاريع.
...
#### استثناء لنقطة نهاية submissions
تم **إعادة تسمية** نقطة النهاية `/assets/{asset_uid}/submissions/` في `v2`. بالإضافة إلى تحديث المسار الأساسي، يجب أيضاً تغيير اسم نقطة النهاية من `submissions` إلى `data`:
...
<sup>1</sup> يمكن أن يكون `{id}` المعرّف الصحيح للإرسال أو `root_uuid` الخاص به.
...
<p class="note">
  <b>ملاحظة:</b> بنية الاستجابة متشابهة تقريباً، <strong>باستثناء أن <code>v2</code> يقدم ترقيم الصفحات</strong>. إذا كان لديك أكثر من 1,000 إرسال، ستحتاج إلى متابعة عنوان URL في `next` لاسترجاع الصفحات التالية.
</p>
